...
def select_labor_days(days, event_start_date, event_end_date):
    ...
    for idx, day in enumerate(days, 1):
        status = "(Event day)" if event_start_date <= day <= event_end_date else "(Outside event window)"
...
def schedule_labor_for_event(selected_event):
    ...
    event_start = selected_event.Start.astimezone(pytz.timezone("America/Chicago")).date()
    event_end = selected_event.End.astimezone(pytz.timezone("America/Chicago")).date()
    print(f"\nThe actual event '{selected_event.Subject}' starts on {event_start} and ends on {event_end}.")

    # Getting labor start and end dates
    labor_start_date = get_date_from_user("When do you want to start scheduling labor? (format: YYYY-MM-DD): ")
    labor_end_date = get_date_from_user("Until when do you want to schedule labor? (format: YYYY-MM-DD): ")

    # Generate a list of days for which labor needs to be scheduled
    days_for_labor = generate_labor_days_list(labor_start_date, labor_end_date, event_start, event_end)
    ...
    # Define the timezone once at the top so you can reuse it
    local_tz = pytz.timezone('America/Chicago')
    ...
    for day, details in labor_schedule.items():
        ...
        event_subject = ", ".join([f"{count} x {position}" for position, count in details['positions'].items()])
        create_calendar_event(labor_calendar, event_subject, event_start, event_end)

    # Ask if the user wants to schedule another shift
    another_shift = input("Would you like to schedule another shift for a different position or day for the same event? (Yes/No): ").strip().lower()
