...
#    grouped = df.groupby(["Event Start Date", "Account"]).agg(
#        EarliestStartTime=("Event Start Time 12 Hour", "min"),
#        LatestEndTime=("Event End Time 12 Hour", "max")
#    ).reset_index()
#
#    for index, row in grouped.iterrows():
#        event_start_date = pd.Timestamp(row["Event Start Date"]).to_pydatetime()
#        event_start_time_str = row["EarliestStartTime"]
#        event_end_time_str = row["LatestEndTime"]
#
#        event_start_time_dt = pd.Timestamp(
#            event_start_date.strftime("%Y-%m-%d") + " " + event_start_time_str
#        ).to_pydatetime()
#        event_end_time_dt = pd.Timestamp(
#            event_start_date.strftime("%Y-%m-%d") + " " + event_end_time_str
#        ).to_pydatetime()
#
#        # Convert to UTC
#        local_tz = pytz.timezone(
#            "America/Chicago"
#        )  # Replace with the appropriate local timezone
#        event_start_time_utc = local_tz.localize(event_start_time_dt).astimezone(
#            pytz.UTC
#        )
